...
- (void)testAddDuplicateNamedSubscriptionWillThrow {
    ...
    [subs update:^{
        ...
        RLMAssertThrowsWithReason([subs addSubscriptionWithClassName:Person.className
                                                    subscriptionName:@"person_age"
                                                               where:@"age > 15"],
                                  @"Cannot duplicate a subscription, adding a subscription with the same name and query will throw. If you meant to update the subscription please use the `update` or `add` method with a new query.");
    }];
